...
- `.ground`: label of the electrical ground
...
- `.formatVersion`: version of the New York (NY) format in which the recordings have been stored.
"""
struct infoDB
    ...
    os                  :: Int                              # trial offset (in samples)
...
loadNYdb is an internal function of [infoNYdb](@ref)

**See Also** 

(xxx put link to FileSytem.getFilesInDir)
...
The only argument (`dbDir`) is the directory holding all files of a database (.npz and .yaml).
...
function selectDB(rootDir       :: String, 
                  ...
                  summary       :: Bool = true)
```
Return a list of infoDB structures.
...
For MI and ERP databases, return only databases featuring the class labels given as kwarg `classes`.

Typical MI class labels are: "left_hand", "right_hand", "feet", "rest", "both_hands", and "tongue".

`minTrials` is a threshold used to exclude sessions with too few trials per class (optional).
Any session that does not meet the minimum number of trials in **at least one class** will be removed 
from the `infoDB.files` field.
For example, if the original database contains ten sessions, and two of them have fewer trials than 
`minTrials` in at least one class, then `infoDB.files` will contain only the 8 remaining sessions 
that meet the threshold criteria.

`summary` is a boolean keyword argument (default = `true`) that controls whether a summary table of 
the selected databases is returned.
...
or any other relevant index across databases. The goal is to balance the contribution of different databases 
and the different subjects therein, considering both the number of unique subjects in each database
and the fact that the number of session for each subject may be different.

The weight assigned to each session is inversely proportional to the square root of the number of unique subjects 
in the database and to the squqre root of the number of sessions available for the same subject.
